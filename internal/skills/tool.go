...
// Tool represents a supported AI coding tool that can consume the installed skills.
type Tool struct {
	Name        string
	Description string
	Generate    func(targetDir string, selected []Skill) error
}
...
func generateOpenCode(targetDir string, selected []Skill) error {
	path := filepath.Join(targetDir, "AGENTS.md")
...
func agentsIndex(selected []Skill) string {
...
func combinedContent(selected []Skill) string {
	var sb strings.Builder
